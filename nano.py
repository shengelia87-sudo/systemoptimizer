...
class MemoryAdapter(Protocol):
    def store(self, kind: str, payload: dict[str, Any], *, ttl: timedelta | None = None) -> str: ...
...
class SkillManager:
    def __init__(
        self, registry: ToolsRegistry, skills_dir: str, constitution: Constitution, fs: SafeFS
    ) -> None:
    ...
    def install_from_code(self, code: str, suggested_name: str | None = None) -> dict[str, Any]:
...
def _ensure_fallback_ocr(tools: ToolsRegistry, fs: SafeFS) -> None:
    ...
    try:
        tools.register(
            FunctionTool("skill:ocr", "Fallback OCR stub: echoes filename and size", _ocr)
        )
    except ValueError:
        pass
...
class CognitiveEngine:
    ...
    def _plan(self, task: str, goals: list[Goal]) -> list[PlanStep]:
        steps: list[PlanStep] = []
        mk = lambda d, a, deps=None: PlanStep(
            id=str(uuid.uuid4()), desc=d, accept_criteria=a, depends_on=list(deps or [])
        )
        s1 = mk(
        ...
        s4 = mk(
            "Curriculum: JavaScript Fundamentals", ["Pass 15/15 JS tests (browserless)"], [s1.id]
        )
        s5 = mk(
    ...
    def _engineer_cycle(self, step: PlanStep) -> tuple[bool, dict[str, Any]]:
        ...
        for loop in range(3):
            ...
            proposal = self._repair(
                step,
                proposal,
                reason=f"test_fail rc={test.get('rc')} stderr={test.get('stderr')} out={test.get('stdout')}",
            )
...
class AutoLearner:
    def __init__(
        self, mem: MemoryAdapter, sm: SkillManager, fs: SafeFS, constitution: Constitution
    ) -> None:
        self.mem = mem
        self.sm = sm
        self.fs = fs
    ...
    def propose_and_build(self, *, max_skills: int = 3) -> list[dict[str, Any]]:
        ...
        for it in items:
            ...
            if ents.get("numbers"):
                stats["has_numbers"] += 1
        if not sample_texts:
            sample_texts = [it.get("raw_excerpt", "") for it in items if it.get("raw_excerpt")] or [
                "sample"
            ]
...
def run_gui() -> int:
    ...
    mem.store(
        kind="note",
        payload={"text": "The system serves the wellbeing of Zurab and Nano Shengelia."},
    )
    ...
    def on_run() -> None:
        task = (
            task_var.get().strip() or "Default: Activate learning curriculum and run engineer cycle"
        )
        ep = eng.run(
            task, goals=[Goal(text="Activate learning curriculum"), Goal(text="Run engineer cycle")]
        )
        out.delete("1.0", "end")
...
def _run_engineer_on_file(
    ...
    *,
    ...
    mem = InMemoryKV()
    eng = CognitiveEngine(
        memory=mem, config=CognitiveConfig(time_limit=time_limit, max_output_bytes=max_output_bytes)
    )
    goals: list[str] = []
    ...
    r = eng.tools.call(
        "sandbox_runner", code=snippet, time_limit=time_limit, max_output_bytes=max_output_bytes
    )
    if r.get("stdout"):
...
def _perception_ops(args: argparse.Namespace, *, fs: SafeFS) -> int | None:
    ...
    if getattr(args, "ingest_dir", None):
        ...
        # exts -> {'txt','md',...}
        exts = {
            e.strip().lower().lstrip(".") for e in (args.ingest_exts or "").split(",") if e.strip()
        }
        results: list[dict[str, Any]] = []
        considered = 0
    ...
    if args.export_knowledge:
        path = fs.join(
            args.export_knowledge if os.path.isabs(args.export_knowledge) else args.export_knowledge
        )
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log-file", type=str, default=None, help="Optional path to also write logs as JSON/text."
    )

    # Engineer flags
    parser.add_argument("--nano", action="store_true", help="Enable NanoPolicy for the engineer.")
    parser.add_argument(
        "--no-autofix", action="store_true", help="Disable auto-patching; analysis only."
    )
    parser.add_argument("--iters", type=int, default=3, help="Max self-heal iterations.")
    parser.add_argument(
        "--target-score", type=float, default=None, help="Target score for engineer (e.g., 0.95)."
    )
    parser.add_argument(
        "--analyze-file",
        type=str,
    ...
    parser.add_argument(
        "--skills-dir", type=str, default=None, help="Directory for user skills (plugins)."
    )
    parser.add_argument(
        "--submit-skill",
        type=str,
    ...
    parser.add_argument(
        "--ingest-text", type=str, default=None, help="Ingest raw text into knowledge store."
    )
    parser.add_argument(
        "--ingest-file",
        type=str,
    ...
    parser.add_argument(
        "--ingest-image", type=str, default=None, help="Ingest an image via OCR skill (skill:ocr)."
    )
    ...
    parser.add_argument(
        "--compact-kv", action="store_true", help="Compact knowledge/mem.jsonl JSONL file and exit."
    )
    ...
    parser.add_argument(
        "--autolearn", dest="autolearn", action="store_true", help="Enable AutoLearner after runs."
    )
    parser.add_argument(
        "--no-autolearn", dest="autolearn", action="store_false", help="Disable AutoLearner."
    )
    parser.set_defaults(autolearn=None)
    parser.add_argument(
        "--autolearn-now", action="store_true", help="Run AutoLearner immediately then exit."
    )
    parser.add_argument(
        "--seed-text", type=str, default=None, help="Optional text to ingest before autolearn-now."
    )
...
def _selftests() -> int:
    ...
    # accept tags logic
    assert eng._accepts(
        PlanStep(id="1", desc="Curriculum: JavaScript Fundamentals", accept_criteria=["Pass"]),
        "OK:JSFundamentals",
    )
...
def main(argv: list[str]) -> int:
    ...
    fs = SafeFS(
        root_dir,
        constitution_path=(os.path.abspath(args.constitution) if args.constitution else None),
    )
    if args.init_root:
        _init_root_tree(fs)
    if args.compact_kv:
    ...
    if args.autolearn_now:
        ...
        eng = CognitiveEngine(
            ...
            config=CognitiveConfig(
                time_limit=args.timeout, max_output_bytes=args.max_bytes, verbose=args.verbose
            ),
        )
