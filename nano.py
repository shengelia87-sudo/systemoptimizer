# nano.py
"""
CogMind v3.2.1 — MAX+++ AutoLearner + Persistent DiskKV, Integrated "Engineer-Mind" + Nano Constitution + SafeFS
...
from typing import Any, Dict, List, Optional, Protocol, Iterable, Tuple, Set, Callable
...
MAX_INGEST_BYTES = int(os.environ.get("COGMIND_MAX_INGEST_BYTES", str(16 * 1024 * 1024)))
...
class MemoryAdapter(Protocol):
    def store(self, kind: str, payload: Dict[str, Any], *, ttl: Optional[timedelta] = None) -> str: ...
    def recall(self, record_id: str) -> Optional[Dict[str, Any]]: ...
    def search(self, query: str, *, k: int = 8, kind: Optional[str] = None) -> List[Dict[str, Any]]: ...
    def summarize(self, items: Iterable[Dict[str, Any]], *, max_chars: int = 1000) -> str: ...
...
class InMemoryKV(MemoryAdapter):
    ...
    def store(self, kind: str, payload: Dict[str, Any], *, ttl: Optional[timedelta] = None) -> str:
        rid = str(uuid.uuid4())
        record = dict(payload)
        record.update({"id": rid, "kind": kind, "created_at": datetime.utcnow().isoformat(), "ttl": ttl.total_seconds() if ttl else None})
    ...
    def search(self, query: str, *, k: int = 8, kind: Optional[str] = None) -> List[Dict[str, Any]]:
        q = (query or "").lower()
        items = [{"id": rid, **data} for rid, data in self._store.items() if (kind is None or data.get("kind") == kind)]
        ...
        return items[:k]
    def summarize(self, items: Iterable[Dict[str, Any]], *, max_chars: int = 1000) -> str:
        parts: List[str] = []; used = 0
        for it in items:
            chunk = it.get("text") or it.get("content") or json.dumps(it, ensure_ascii=False)
...
class SafeFS:
    ...
    """
    def __init__(self, root_dir: str, constitution_path: Optional[str] = None) -> None:
        self.root = os.path.abspath(root_dir)
        self.constitution_path = os.path.abspath(constitution_path) if constitution_path else None
...
class DiskKV(MemoryAdapter):
    ...
    def store(self, kind: str, payload: Dict[str, Any], *, ttl: Optional[timedelta] = None) -> str:
        ...
        record = dict(payload)
        record.update({
            "id": rid, "kind": kind, "created_at": datetime.utcnow().isoformat(),
    ...
    def search(self, query: str, *, k: int = 8, kind: Optional[str] = None) -> List[Dict[str, Any]]:
        q = (query or "").lower()
        items = [rec for rec in self._store.values() if (kind is None or rec.get("kind") == kind)]
        def score(it: Dict[str, Any]) -> float:
            text = json.dumps(it, ensure_ascii=False).lower()
            base = 1.0 if q in text else 0.0
            terms = set([w for w in re.split(r"[^\w]+", q) if w])
    ...
    # ---- Utility for exports ----
    def iter_items(self, kind: Optional[str] = None) -> Iterable[Dict[str, Any]]:
...
class ToolsRegistry:
    ...
    def call(self, name: str, **kwargs: Any) -> Dict[str, Any]:
        if name not in self._tools:
            sname = f"skill:{name}"
            if sname in self._tools: return self._tools[sname].use(**kwargs)
...
class PyLintLite:
    ...
    def use(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        warnings = []
        for b in banned:
            if b in code: warnings.append(f"banned:{b}")
        long_lines = [i+1 for i, l in enumerate(code.splitlines()) if len(l) > 120]
        if long_lines: warnings.append(f"long_lines:{long_lines[:10]}")
    ...
    def use(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                return {"ok": proc.returncode == 0, "rc": proc.returncode, "stdout": out, "stderr": err}
...
class Constitution:
    allowed_imports: Set[str] = field(default_factory=lambda: {
        "math", "statistics", "random", "re", "json", "typing",
        "dataclasses", "functools", "itertools", "collections"
    })
    banned_imports: Set[str] = field(default_factory=lambda: {
        "os", "sys", "subprocess", "socket", "http", "urllib", "ssl",
        "ctypes", "multiprocessing", "threading", "pathlib", "shutil"
    })
    banned_calls: Set[str] = field(default_factory=lambda: {"eval", "exec", "__import__", "open"})
    ...
    @classmethod
    def load(cls, path: Optional[str]) -> "Constitution":
        if path and os.path.isfile(path):
            try:
                with open(path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                return cls(
                    allowed_imports=set(data.get("allowed_imports", [])) or cls().allowed_imports,
                    banned_imports=set(data.get("banned_imports", [])) or cls().banned_imports,
                    banned_calls=set(data.get("banned_calls", [])) or cls().banned_calls,
                    max_top_level_statements=int(data.get("max_top_level_statements", 200)),
    ...
    def check_code(self, code: str) -> List[str]:
        ...
        for node in body:
            ...
            elif isinstance(node, (ast.FunctionDef, ast.ClassDef, ast.Assign, ast.Import, ast.ImportFrom, ast.AnnAssign, ast.If)):
                if isinstance(node, ast.If):
                    test_ok = isinstance(node, ast.Compare) and any(
                        isinstance(c, ast.Constant) and c.value == "__main__" for c in getattr(node, "comparators", [])
                    )
        ...
        for n in ast.walk(tree):
            if isinstance(n, ast.Import):
                for a in n.names:
                    ...
                    if mod in self.banned_imports or (self.allowed_imports and mod not in self.allowed_imports):
            ...
            if isinstance(n, ast.Call):
                ...
                if isinstance(n.func, ast.Name):
                    ...
                    callee = n.func.attr
                    if isinstance(n.func.value, ast.Name) and n.func.value.id in {"os", "subprocess"}:
                        violations.append(f"banned_attr_call:{n.func.value.id}.{n.func.attr}")
...
class SkillManager:
    def __init__(self, registry: ToolsRegistry, skills_dir: str, constitution: Constitution, fs: SafeFS) -> None:
    ...
    def install_from_file(self, path: str) -> Dict[str, Any]:
        ...
        return self.install_from_code(code, suggested_name=os.path.splitext(os.path.basename(path))[0])

    def install_from_code(self, code: str, suggested_name: Optional[str] = None) -> Dict[str, Any]:
        violations = self.constitution.check_code(code)
        if violations:
            return {"ok": False, "reason": "constitution_block", "violations": violations}
    ...
    def load_all_and_register(self) -> Dict[str, Any]:
        ...
        for name in self.fs.listdir(self.skills_dir):
            ...
            try:
                ...
                infos.append(self._register_module_skills(module, os.path.splitext(name)[0]))
            except Exception as e:
    ...
    def _register_module_skills(self, module, mod_name: str) -> Dict[str, Any]:
        skills = getattr(module, "SKILLS", None); meta = getattr(module, "SKILL_META", {})
        ...
        return {"ok": True, "module": mod_name, "registered": registered, "replaced": replaced, "already_registered": replaced}
...
def _ensure_fallback_ocr(tools: ToolsRegistry, fs: SafeFS) -> None:
    ...
    try:
        tools.register(FunctionTool("skill:ocr", "Fallback OCR stub: echoes filename and size", _ocr))
    except ValueError:
        pass
...
class CognitiveEngine:
    def __init__(self, *, memory: MemoryAdapter, tools: Optional[ToolsRegistry] = None, config: Optional[CognitiveConfig] = None) -> None:
        self.memory = memory
        self.wm = WorkingMemory()
        self.tools = tools or ToolsRegistry()
        self.config = config or CognitiveConfig()
        for t in (PyLintLite(), SandboxRunner()):
            try: self.tools.register(t)
    ...
    def run(self, task: str, *, goals: Optional[List[Goal]] = None) -> Episode:
        ...
        self._log(ep, "plan", json.dumps([s.__dict__ for s in steps], ensure_ascii=False))
        for step in steps:
            if not all(self._get_step(steps, d).status == "OK" for d in step.depends_on):
                continue
            step.status = "RUNNING"
            ...
            self._log(ep, role, f"Step status={step.status}; evidence={json.dumps(evidence, ensure_ascii=False)}")
            if step.status != "OK":
                self._log(ep, "critic", "Stopping plan — previous step failed.")
                break
        if self.config.auto_store_context:
            self.memory.store(kind="episode", payload={"topic": task, "trace": ep.text()})
    ...
    def _plan(self, task: str, goals: List[Goal]) -> List[PlanStep]:
        steps: List[PlanStep] = []
        mk = lambda d, a, deps=None: PlanStep(id=str(uuid.uuid4()), desc=d, accept_criteria=a, depends_on=list(deps or []))
        s1 = mk("Curriculum: Python Core -> Syntax, Types, Functions, OOP", ["Pass 30/30 unit tests"], [])
        ...
        s4 = mk("Curriculum: JavaScript Fundamentals", ["Pass 15/15 JS tests (browserless)"], [s1.id])
        s5 = mk("Build: Engineer scaffold for self-improving code assistant", ["Linter OK", "Sandbox tests pass"], [s1.id, s2.id, s3.id, s4.id])
        s6 = mk("Self-Optimization: Auto-learn skills from knowledge", ["≥0 or more skills registered"], [s1.id])
    ...
    def _engineer_cycle(self, step: PlanStep) -> Tuple[bool, Dict[str, Any]]:
        ...
        for loop in range(3):
            ...
            if not lint.get("ok"):
                proposal = self._repair(step, proposal, reason=f"lint:{lint.get('warnings')}")
                continue
            ...
            if test.get("ok") and test.get("rc") == 0 and self._accepts(step, test.get("stdout", "")):
                return True, evidence
            proposal = self._repair(step, proposal, reason=f"test_fail rc={test.get('rc')} stderr={test.get('stderr')} out={test.get('stdout')}")
        return False, evidence

    def _propose(self, step: PlanStep) -> Dict[str, Any]:
        return {"goal": step.desc, "strategy": "tiny unit-tests -> minimal impl -> improve"}

    def _build(self, step: PlanStep, proposal: Dict[str, Any]) -> str:
        if step.desc.startswith("Curriculum: Python"): return self._code_curriculum_python()
        if step.desc.startswith("Curriculum: Bash"): return self._code_curriculum_bash()
        if step.desc.startswith("Curriculum: SQL"): return self._code_curriculum_sql()
        if step.desc.startswith("Curriculum: JavaScript"): return self._code_curriculum_js()
        if step.desc.startswith("Build: Engineer scaffold"): return self._code_engineer_scaffold()
        return "print('noop')\n"
...
"""
    ...
    def _accepts(self, step: PlanStep, out: str) -> bool:
        want_tags: List[str] = []
        if step.desc.startswith("Curriculum: Python"): want_tags = ["OK:PythonCore", "OK:PythonCore:extra"]
        if step.desc.startswith("Curriculum: Bash"): want_tags = ["OK:BashEssentials"]
        if step.desc.startswith("Curriculum: SQL"): want_tags = ["OK:SQLBasics"]
        if step.desc.startswith("Curriculum: JavaScript"): want_tags = ["OK:JSFundamentals"]
        if step.desc.startswith("Build: Engineer scaffold"): want_tags = ["OK:EngineerScaffold"]
        return all(tag in out for tag in want_tags)

    def _repair(self, step: PlanStep, proposal: Dict[str, Any], *, reason: str) -> Dict[str, Any]:
        proposal = dict(proposal); proposal["last_reason"] = reason; return proposal
...
"""
    ...
    def _code_engineer_scaffold(self) -> str:
        return (
            ...
            "    def evaluate_code(self, code, goals=None, auto_fix=True, max_iters=3, nano_mode=False):\n"
...
class KnowledgeCompressor:
    ...
    WORD_RE = re.compile(r"[A-Za-z0-9\-_\u10A0-\u10FF\u2D00-\u2D2F\u1C90-\u1CBF]+", re.UNICODE)
    ...
    NUMBER_RE = re.compile(r'(?<!\d{4}-)(?<!-\d-)(?<!-\d{2}-)(?<!\d)[-+]?\d+(?:\.\d+)?(?!\d)')
    ...
    def split_sections(self, text: str) -> Dict[str, str]:
        ...
        for ln in lines:
            ...
            if s.startswith(("def ", "class ", "import ", "from ")):
                code.append(ln)
            elif ("|" in ln and ln.count("|") >= 2) or ("," in ln and len(ln.split(",")) > 2) or ("\t" in ln):
                tables.append(ln)
            else:
                body.append(ln)
        return {"code": "\n".join(code), "tables": "\n".join(tables), "body": "\n".join(body)}
    def keyphrases(self, tokens: List[str], top_k: int = 20) -> List[str]:
        stop = set(self._stopwords()); grams = collections.Counter()
        toks = [t for t in tokens if t not in stop and len(t) >= 3]
        for i in range(len(toks)):
            grams[toks[i]] += 1
            if i+1 < len(toks): grams[toks[i]+" "+toks[i+1]] += 1
            if i+2 < len(toks): grams[" ".join(toks[i:i+2+1])] += 1
        return [w for w,_ in grams.most_common(top_k)]
    def summarize(self, text: str, max_sentences: int = 7) -> str:
        sents = re.split(r"(?<=[.!?])\s+", text)
        if len(sents) <= max_sentences: return " ".join(sents).strip()
        toks = self.tokenize(text); freq = collections.Counter(toks); scored=[]
        for s in sents:
    ...
    def entities(self, text: str) -> Dict[str, List[str]]:
        ...
        return {"names": names, "dates": dates, "links": links, "emails": emails, "numbers": numbers}
    def compress(self, title: str, text: str) -> CompressedDoc:
        norm = self.normalize(text); toks = self.tokenize(norm)
        sh = self.simhash64(toks); sections = self.split_sections(norm)
        summary = self.summarize(norm, max_sentences=7); phrases = self.keyphrases(toks, top_k=20); ents = self.entities(norm)
        return CompressedDoc(
            id=str(uuid.uuid4()), title=title or (norm[:40] + ("..." if len(norm)>40 else "")),
            created_at=datetime.utcnow().isoformat(), simhash_hex=f"{sh:016x}",
            size_bytes=len(norm.encode("utf-8")), tokens=len(toks),
            summary=summary, keyphrases=phrases, entities=ents, sections=sections,
        )
    def _stopwords(self) -> Iterable[str]:
        return {"the","a","an","and","or","to","of","in","on","for","by","with","is","are","was","were","be","this",
                "that","it","as","at","from","we","you","i","your","our","their","not","but","if","then","so",
                "do","does","did","can","could","should","would","about","into","over","under","out","up","down"}

class KnowledgeQuery:
    def __init__(self, memory: MemoryAdapter) -> None:
        self.memory = memory
    def hamming(self, h1_hex: str, h2_hex: str) -> int:
        x = int(h1_hex, 16) ^ int(h2_hex, 16); return bin(x).count("1")
    def find_near(self, sketch_hex: str, *, max_dist: int = 8, k: int = 5) -> List[Dict[str, Any]]:
        items = self.memory.search("", k=1000, kind="knowledge"); out: List[Tuple[int, Dict[str, Any]]] = []
        for it in items:
            sh = it.get("simhash_hex"); 
            if not sh: continue
            d = self.hamming(sketch_hex, sh)
            if d <= max_dist: out.append((d, it))
        out.sort(key=lambda x: x[0]); return [it for _, it in out[:k]]

class PerceptionEngine:
    def __init__(self, memory: MemoryAdapter, tools: ToolsRegistry) -> None:
        self.memory = memory; self.tools = tools; self.comp = KnowledgeCompressor()
    ...
    def ingest_text(self, text: str, *, title: str = "") -> Dict[str, Any]:
        ...
        rid = self.memory.store(kind="knowledge", payload={
            "title": doc.title, "created_at": doc.created_at, "simhash_hex": doc.simhash_hex,
            "size_bytes": doc.size_bytes, "tokens": doc.tokens, "summary": doc.summary,
            "keyphrases": doc.keyphrases, "entities": doc.entities, "sections": doc.sections,
            "raw_excerpt": (text[:2000] + ("..." if len(text) > 2000 else "")),
            "near_dup_of": near[0]["id"] if near else None
        })
        return {"ok": True, "id": rid, "simhash_hex": doc.simhash_hex, "tokens": doc.tokens, "near_dup": bool(near)}

    # --- NEW: helper-ები docx/pdf ტექსტის ამოსაღებად ---
    def _extract_docx_text(self, path: str) -> Optional[str]:
        try:
    ...
    def _extract_pdf_text(self, path: str, max_pages: int = 200) -> Optional[str]:
        try:
    ...
    def ingest_file(self, path: str, *, title: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if sz > MAX_INGEST_BYTES:
                return {"ok": False, "error": "file_too_large", "size_bytes": int(sz), "limit_bytes": int(MAX_INGEST_BYTES)}
        except Exception:
        ...
        try:
            ...
            if len(raw) > MAX_INGEST_BYTES:
                return {"ok": False, "error": "file_too_large", "size_bytes": len(raw), "limit_bytes": int(MAX_INGEST_BYTES)}
    ...
    def ingest_image(self, path: str, *, title: Optional[str] = None) -> Dict[str, Any]:
        try:
            toolres = self.tools.call("ocr", image_path=path)
        except KeyError:
            return {"ok": False, "error": "ocr_skill_missing", "hint": "Install 'ocr' skill returning {'ok':True,'text':...}"}
        if not toolres.get("ok"): return {"ok": False, "error": toolres.get("error", "ocr_failed")}
        return self.ingest_text(toolres.get("text", ""), title=title or os.path.basename(path))
...
class AutoLearner:
    def __init__(self, mem: MemoryAdapter, sm: SkillManager, fs: SafeFS, constitution: Constitution) -> None:
        self.mem = mem; self.sm = sm; self.fs = fs; self.constitution = constitution
    ...
    def propose_and_build(self, *, max_skills: int = 3) -> List[Dict[str, Any]]:
        ...
        stats = {"has_tables":0, "has_urls":0, "has_emails":0, "has_dates":0, "has_numbers":0}
        for it in items:
            for p in it.get("keyphrases", []): phrases[p] += 1
            sec = it.get("sections", {}) or {}
            body = sec.get("body",""); tables = sec.get("tables",""); code = sec.get("code","")
            if body: sample_texts.append(body)
            if tables: sample_texts.append(tables); stats["has_tables"] += 1
            ents = it.get("entities",{}) or {}
            if ents.get("links"): stats["has_urls"] += 1
            if ents.get("emails"): stats["has_emails"] += 1
            if ents.get("dates"): stats["has_dates"] += 1
            if ents.get("numbers"): stats["has_numbers"] += 1
        if not sample_texts:
            sample_texts = [it.get("raw_excerpt","") for it in items if it.get("raw_excerpt")] or ["sample"]
        ...
        if top_phrases:
            candidates.append(("phrase_counter", self._skill_phrase_counter(top_phrases),
                               {"kind":"phrase_counter","phrases":top_phrases}))
        candidates.append(("number_stats", self._skill_number_stats(), {"kind":"number_stats"}))
        candidates.append(("regex_extract", self._skill_regex_extract(), {"kind":"regex_extract"}))
        candidates.append(("table_to_json", self._skill_table_to_json(), {"kind":"table_to_json"}))
        candidates.append(("simhash_fingerprint", self._skill_simhash_fingerprint(), {"kind":"simhash_fingerprint"}))

        results: List[Dict[str, Any]] = []
        for name, code, meta in candidates:
            info = self.sm.install_from_code(code, suggested_name=f"auto_{name}")
            if not info.get("ok"):
                results.append({"ok": False, "skill": name, "install": info}); continue
            tool_name = f"skill:{name}"
            score = 0.0; trials = 0; ok_count = 0
            for sample in sample_texts[:5]:
                trials += 1
                try:
                    if name == "phrase_counter":
                        res = self.sm.registry.call(tool_name, text=sample, phrases=top_phrases)
                        util = sum(res.get("counts", {}).values()) if res.get("ok") else 0
                        ...
                        util = sum(len(v) for (k,v) in res.get("matches", {}).items()) if res.get("ok") else 0
                        ...
                        res = {"ok": False}
                        util = 0
                    if res.get("ok"): ok_count += 1
                    score += util
                except Exception:
                    util = 0
            avg = (score / max(1, trials))
            results.append({"ok": True, "skill": name, "installed": info, "avg_utility": avg, "ok_rate": ok_count/max(1,trials), "meta": meta})

        keep = sorted([r for r in results if r.get("ok")], key=lambda r: r.get("avg_utility", 0.0), reverse=True)[:max_skills]
    ...
    def _skill_phrase_counter(self, phrases: List[str]) -> str:
        ...
        code = (
            ...
            "    terms = [w.lower() for w in re.findall(r'[A-Za-z0-9\\-_\\u10A0-\\u10FF\\u2D00-\\u2D2F\\u1CBF]+', text or '')]\n"
    ...
    def _skill_number_stats(self) -> str:
        return (
            ...
            "    nums = [float(x) for x in re.findall(r'(?<!\\d{4}-)(?<!-\\d-)(?<!-\\d{2}-)(?<!\\d)[-+]?\\d+(?:\\.\\d+)?(?!\\d)', text or '')]\n"
            "    if not nums:\n"
    ...
    def _skill_table_to_json(self) -> str:
        return (
            ...
            "    if '|' in line and line.count('|')>=2: parts = [p.strip() for p in line.split('|') if p.strip()!='']\n"
...
def run_gui() -> int:
    ...
    mem.store(kind="note", payload={"text": "The system serves the wellbeing of Zurab and Nano Shengelia."})

    root_w = tk.Tk(); root_w.title("CogMind v3.2.1")
    frm = ttk.Frame(root_w, padding=10); frm.pack(fill="both", expand=True)
    ttk.Label(frm, text="Task:").grid(row=0, column=0, sticky="w")
    task_var = tk.StringVar()
    ent = ttk.Entry(frm, textvariable=task_var, width=80); ent.grid(row=0, column=1, sticky="we")
    frm.columnconfigure(1, weight=1)
    btn = ttk.Button(frm, text="Run"); btn.grid(row=0, column=2, padx=5)
    ttk.Label(frm, text="Paste text to ingest:").grid(row=2, column=0, columnspan=3, sticky="w", pady=(10,0))
    ingest_box = ScrolledText(frm, height=10, wrap="word"); ingest_box.grid(row=3, column=0, columnspan=3, sticky="nsew")
    frm.rowconfigure(3, weight=1)
    ingest_btn = ttk.Button(frm, text="Ingest"); ingest_btn.grid(row=4, column=2, sticky="e", pady=(6,0))
    out = ScrolledText(frm, height=18, wrap="word"); out.grid(row=5, column=0, columnspan=3, sticky="nsew", pady=(10,0))
    frm.rowconfigure(5, weight=1)

    def on_run() -> None:
        task = task_var.get().strip() or "Default: Activate learning curriculum and run engineer cycle"
        ep = eng.run(task, goals=[Goal(text="Activate learning curriculum"), Goal(text="Run engineer cycle")])
        out.delete("1.0", "end"); out.insert("end", "========== EPISODE TRACE ==========\n")
        out.insert("end", ep.text()); out.insert("end", "\n\nFinished: " + str(ep.finished_at))
    def on_ingest() -> None:
        data = ingest_box.get("1.0", "end").strip()
        if not data: out.insert("end", "\n[warn] Nothing to ingest.\n"); return
        res = perceiver.ingest_text(data, title="GUI-paste")
        out.insert("end", f"\n[ingest] ok={res.get('ok')} id={res.get('id')} tokens={res.get('tokens')} near_dup={res.get('near_dup')}\n")

    btn.configure(command=on_run); ingest_btn.configure(command=on_ingest)
    ent.bind("<Return>", lambda e: on_run()); root_w.mainloop(); return 0

# ============================
#   Engineer/Skills/Perception CLI ops
# ============================

def _run_engineer_on_file(path: str, *, nano: bool, no_autofix: bool, iters: int, target_score: Optional[float],
                          time_limit: int = 5, max_output_bytes: int = 4000) -> int:
    mem = InMemoryKV(); eng = CognitiveEngine(memory=mem, config=CognitiveConfig(time_limit=time_limit, max_output_bytes=max_output_bytes))
    goals: List[str] = []; 
    if target_score is not None: goals.append(f"target_score>={target_score}")
    try:
        with open(path, "r", encoding="utf-8") as f: src = f.read()
    except Exception as e:
        print("ERROR: cannot read file:", e); return 2
    engineer_code = eng._code_engineer_scaffold()
    import json as _json
    snippet = engineer_code + "\n" + (
        "import json\n"
        f"SRC = {_json.dumps(src)}\n"
        "e = EngineerScaffold()\n"
        f"res = e.evaluate_code(SRC, goals=" + _json.dumps(goals) +
        f", auto_fix={str(not no_autofix)}, max_iters={int(iters)}, nano_mode={str(nano)})\n"
        "print(json.dumps(res, ensure_ascii=False))\n"
    )
    r = eng.tools.call("sandbox_runner", code=snippet, time_limit=time_limit, max_output_bytes=max_output_bytes)
    if r.get("stdout"): print(r["stdout"].strip())
    if r.get("stderr"): print(r["stderr"].strip(), file=sys.stderr)
        ...
        try:
            try:
                ...
                if sz > MAX_STDIN_BYTES:
                    print(json.dumps({"ok": False, "error": "skill_args_file_too_large", "size_bytes": int(sz), "limit_bytes": int(MAX_STDIN_BYTES)}, ensure_ascii=False))
                    sys.exit(2)
            except Exception:
                pass
            ...
            if len(raw) > MAX_STDIN_BYTES:
                print(json.dumps({"ok": False, "error": "skill_args_file_too_large", "size_bytes": len(raw), "limit_bytes": int(MAX_STDIN_BYTES)}, ensure_ascii=False))
                sys.exit(2)
            ...
            kw = json.loads(text)
            if not isinstance(kw, dict): raise ValueError("skill_args_must_be_object")
            return kw
        except Exception as e:
            print("ERROR: invalid --skill-args-file:", e); sys.exit(2)
    ...
    if getattr(args, "skill_args_b64", None):
        try:
            ...
            if len(b) > MAX_STDIN_BYTES:
                print(json.dumps({"ok": False, "error": "skill_args_b64_too_large", "size_bytes": len(b), "limit_bytes": int(MAX_STDIN_BYTES)}, ensure_ascii=False))
                sys.exit(2)
            ...
            print("ERROR: invalid --skill-args-b64:", e); sys.exit(2)
        ...
        try:
            ...
            if raw_bytes is not None:
                if len(raw_bytes) > MAX_STDIN_BYTES:
                    print(json.dumps({"ok": False, "error": "skill_args_stdin_too_large", "size_bytes": len(raw_bytes), "limit_bytes": int(MAX_STDIN_BYTES)}, ensure_ascii=False))
                    sys.exit(2)
                last_err: Optional[Exception] = None
                for enc in ("utf-8-sig", "utf-16", "utf-16-le", "utf-16-be", "utf-8"):
                    try:
            ...
            print("ERROR: invalid --skill-args-stdin:", e); sys.exit(2)
    # plain (BOM-friendly)
    try:
        ...
        print("ERROR: invalid --skill-args JSON:", e); sys.exit(2)

def _validate_skill_io(obj: Any) -> Optional[str]:
...
def _skills_ops(args: argparse.Namespace, *, fs: SafeFS) -> Optional[int]:
    mem = _make_memory(fs); eng = CognitiveEngine(memory=mem)
    ...
    if args.submit_skill:
        info = sm.install_from_file(args.submit_skill); print(json.dumps(info, ensure_ascii=False, indent=2))
    ...
    if args.list_skills:
        ...
        print(json.dumps({"skills": skills}, ensure_ascii=False, indent=2)); return 0
    ...
    if args.use_skill:
        ...
        try:
            ...
            print("ERROR calling skill:", e); return 1
        if args.validate_skill_io:
            verr = _validate_skill_io(res)
            if verr:
                print(json.dumps({"ok": False, "error": f"skill_contract_violation: {verr}", "hint": "Ensure your skill returns {'ok': bool, ...} dict"}, ensure_ascii=False, indent=2))
                return 1
...
def _compact_kv(fs: SafeFS) -> int:
    ...
    if not os.path.isfile(path):
        print("No mem.jsonl to compact at:", path); return 0
    tmp = path + ".tmp"
...
def _perception_ops(args: argparse.Namespace, *, fs: SafeFS) -> Optional[int]:
    mem = _make_memory(fs); eng = CognitiveEngine(memory=mem)
    ...
    if args.ingest_text is not None:
        res = perceiver.ingest_text(args.ingest_text, title=(args.title or "ingest-text"))
        print(json.dumps(res, ensure_ascii=False, indent=2)); return 0 if res.get("ok") else 1
    if args.ingest_file:
    ...
    if getattr(args, "ingest_dir", None):
        ...
        # exts -> {'txt','md',...}
        exts = {e.strip().lower().lstrip(".") for e in (args.ingest_exts or "").split(",") if e.strip()}
        results: List[Dict[str, Any]] = []
        considered = 0
        for dirpath, dirnames, filenames in os.walk(rootdir):
            if args.ingest_skip_hidden:
                # დირექტორიების გაფილტვრა Hidden/System + dot (OS-სპეციფიკურად)
                dirnames[:] = [d for d in dirnames if not _is_hidden_path(os.path.join(dirpath, d))]
            for fn in filenames:
                ...
                if exts and ext not in exts:
                    continue
                considered += 1
                res = perceiver.ingest_file(fpath, title=os.path.relpath(fpath, rootdir))
                results.append({"path": fpath, **res})
        ok_count = sum(1 for r in results if r.get("ok"))
        errors = [{"path": r["path"], "error": r.get("error")} for r in results if not r.get("ok")]
        print(json.dumps({
            "ok": ok_count > 0,
            "ingested": ok_count,
    ...
    if args.knowledge_query:
        ...
        print(json.dumps({"results": qres}, ensure_ascii=False, indent=2)); return 0
    if args.export_knowledge:
        path = fs.join(args.export_knowledge if os.path.isabs(args.export_knowledge) else args.export_knowledge)
        if hasattr(mem, "iter_items"):
            items = list(getattr(mem, "iter_items")("knowledge"))  # type: ignore
        elif hasattr(mem, "_store"):
            items = [v for v in getattr(mem, "_store").values() if v.get("kind")=="knowledge"]  # type: ignore
        else:
            items = []
        content = "\n".join(json.dumps(it, ensure_ascii=False) for it in items)
        fs.write_text(path, (content + ("\n" if content else "")))
        print(f"Exported {len(items)} records to {path}"); return 0
...
def _setup_logging(verbose: bool, json_logs: bool, log_file: Optional[str] = None) -> None:
...
def _parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=("Run CogMind/Nano engine on a task (v3.2.1 MAX+++).\n"
                     "Skill kwargs priority: --skill-args-file > --skill-args-b64 > --skill-args-stdin > --skill-args")
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")

    parser.add_argument("task", nargs="?", default=os.environ.get("COGMIND_TASK"), help="Task to run.")
    parser.add_argument("--interactive", action="store_true", help="Prompt for a task if none provided.")
    parser.add_argument("--selftest", action="store_true", dest="selftest", help="Run built-in self tests and exit.", default=False)
    parser.add_argument("--gui", action="store_true", help="Launch the Tkinter GUI instead of CLI.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logs (INFO/DEBUG).")
    parser.add_argument("--json-logs", action="store_true", help="Emit logs as JSON lines to stdout.")
    parser.add_argument("--log-file", type=str, default=None, help="Optional path to also write logs as JSON/text.")

    # Engineer flags
    parser.add_argument("--nano", action="store_true", help="Enable NanoPolicy for the engineer.")
    parser.add_argument("--no-autofix", action="store_true", help="Disable auto-patching; analysis only.")
    parser.add_argument("--iters", type=int, default=3, help="Max self-heal iterations.")
    parser.add_argument("--target-score", type=float, default=None, help="Target score for engineer (e.g., 0.95).")
    parser.add_argument("--analyze-file", type=str, default=None, help="Analyze a code file with the MAX+++ Engineer and exit.")

    # Sandbox (CLI-configurable)
    parser.add_argument("--timeout", type=int, default=5, help="Sandbox time limit (seconds).")
    parser.add_argument("--max-bytes", type=int, default=4000, help="Max bytes of stdout/stderr to retain from sandbox.")

    # Constitution/skills/perception
    parser.add_argument("--constitution", type=str, default=None, help="Path to constitution JSON file.")
    parser.add_argument("--skills-dir", type=str, default=None, help="Directory for user skills (plugins).")
    parser.add_argument("--submit-skill", type=str, default=None, help="Install a new skill from a .py file and exit.")
    parser.add_argument("--list-skills", action="store_true", help="List registered skills and exit.")
    parser.add_argument("--use-skill", type=str, default=None, help="Run a registered skill by name.")
    parser.add_argument("--skill-args", type=str, default="{}", help="JSON kwargs passed to the skill (lowest priority).")
    parser.add_argument("--skill-args-file", type=str, default=None, help="Path to a JSON file with kwargs for --use-skill (highest priority).")
    parser.add_argument("--skill-args-b64", type=str, default=None, help="Base64-encoded JSON kwargs (2nd priority).")
    parser.add_argument("--skill-args-stdin", action="store_true", help="Read JSON kwargs from stdin (3rd priority).")
    parser.add_argument("--validate-skill-io", action="store_true", help="Validate skill JSON response has minimal contract.")
    parser.add_argument("--ingest-text", type=str, default=None, help="Ingest raw text into knowledge store.")
    parser.add_argument("--ingest-file", type=str, default=None, help="Ingest a text file (UTF-8/UTF-16 auto-detect, size-limited).")
    parser.add_argument("--ingest-image", type=str, default=None, help="Ingest an image via OCR skill (skill:ocr).")
    # >>> NEW ARGS: directory ingest <<<
    parser.add_argument("--ingest-dir", type=str, default=None, help="Ingest all text-like files from a directory (recursive).")
    parser.add_argument("--ingest-exts", type=str, default="txt,md,csv,json,log,py,html,htm,ini,conf,yaml,yml,tsv", help="Comma-separated extensions to include (without dots).")
    parser.add_argument("--ingest-skip-hidden", action="store_true", help="Skip hidden files and folders when using --ingest-dir.")
    # <<< END NEW ARGS >>>
    parser.add_argument("--title", type=str, default=None, help="Optional title for ingestion.")
    parser.add_argument("--knowledge-query", type=str, default=None, help="Query knowledge store.")
    parser.add_argument("--export-knowledge", type=str, default=None, help="Export knowledge store to JSONL (path is constrained to safe root).")
    parser.add_argument("--compact-kv", action="store_true", help="Compact knowledge/mem.jsonl JSONL file and exit.")

    # Safe Root / AutoLearn
    parser.add_argument("--root-dir", type=str, default=r"D:\nano shengelia\systemoptimizer", help="Safe root directory for all writes.")
    parser.add_argument("--init-root", action="store_true", help="Initialize directory tree under --root-dir (skills, knowledge, models, exports).")
    parser.add_argument("--autolearn", dest="autolearn", action="store_true", help="Enable AutoLearner after runs.")
    parser.add_argument("--no-autolearn", dest="autolearn", action="store_false", help="Disable AutoLearner.")
    parser.set_defaults(autolearn=None)
    parser.add_argument("--autolearn-now", action="store_true", help="Run AutoLearner immediately then exit.")
    parser.add_argument("--seed-text", type=str, default=None, help="Optional text to ingest before autolearn-now.")
...
def _selftests() -> int:
    mem = InMemoryKV(); eng = CognitiveEngine(memory=mem)
    # linter must catch banned eval
    lint = eng.tools.call("pylint_lite", code="eval('1')\n")
    assert lint.get("ok") is False and any("banned" in w for w in lint.get("warnings", []))
    # sandbox must execute and echo
    sb = eng.tools.call("sandbox_runner", code="print('hello')\n", time_limit=5, max_output_bytes=4000)
    assert sb.get("ok") and sb.get("rc") == 0 and "hello" in sb.get("stdout", "")
    # accept tags logic
    assert eng._accepts(PlanStep(id="1", desc="Curriculum: JavaScript Fundamentals", accept_criteria=["Pass"]), "OK:JSFundamentals")
    ...
    try:
        ...
        tf.write("UTF16 sample 123".encode("utf-16")); tf.flush(); tf.close()
        res = perceiver.ingest_file(tf.name, title="utf16-test")
        assert res.get("ok"), res
    finally:
        try:
            if tf and tf.name and os.path.exists(tf.name): os.unlink(tf.name)
        except Exception:
            pass
    print("SELFTESTS: OK")
    return 0
...
def _init_root_tree(fs: SafeFS) -> None:
    fs.ensure_dir("skills"); fs.ensure_dir("knowledge"); fs.ensure_dir("models"); fs.ensure_dir("exports")
...
def main(argv: List[str]) -> int:
    ...
    if args.selftest: return _selftests()
    if args.gui: return run_gui()

    constitution = Constitution.load(args.constitution)
    root_dir = args.root_dir or constitution.safe_root or r"D:\nano shengelia\systemoptimizer"
    fs = SafeFS(root_dir, constitution_path=(os.path.abspath(args.constitution) if args.constitution else None))
    if args.init_root: _init_root_tree(fs)
    if args.compact_kv:
        return _compact_kv(fs)

    if args.autolearn_now:
        mem = _make_memory(fs)
        eng = CognitiveEngine(memory=mem, config=CognitiveConfig(time_limit=args.timeout, max_output_bytes=args.max_bytes, verbose=args.verbose))
        sm = SkillManager(eng.tools, args.skills_dir or fs.join("skills"), constitution, fs)
        sm.load_all_and_register()
        _ensure_fallback_ocr(eng.tools, fs)
        perceiver = PerceptionEngine(mem, eng.tools)
        if args.seed_text:
    ...
    if maybe2 is not None: return maybe2
    ...
    task: Optional[str] = args.task
    if args.interactive and not task:
        try: task = input("Enter task: ").strip()
        except EOFError: task = None
    if not task:
    ...
    eng = CognitiveEngine(
        memory=mem,
        config=CognitiveConfig(time_limit=args.timeout, max_output_bytes=args.max_bytes, verbose=args.verbose)
    )
    skills_dir = args.skills_dir or fs.join("skills")
    sm = SkillManager(eng.tools, skills_dir, constitution, fs)
    sm.load_all_and_register()
    ...
    use_autolearn = constitution.allow_autolearn if args.autolearn is None else args.autolearn

    mem.store(kind="note", payload={"text": "The system serves the wellbeing of Zurab and Nano Shengelia."})
    ep = eng.run(task, goals=[Goal(text="Activate learning curriculum"), Goal(text="Run engineer cycle")])
    print("\n========== EPISODE TRACE =========="); print(ep.text()); print("\nFinished:", ep.finished_at)
