...
def _script_accepts(word: str, script: str) -> bool:
    ...
    if script == "ge":
        return has_ge and not has_lat
    if script == "lat":
        return has_lat and not has_ge
    return True
...
def _maybe_repair_mojibake(s: str) -> str:
    ...
    if "áƒ" in s:
        try:
            ...
            def ge_count(t: str) -> int:
                return sum(1 for ch in t if _is_ge(ch))
...
# ---- tokenizer (no regex) ----
def _tokenize(
    text: str, *, join_hyphen: bool = False, allow_sign: bool = True, allow_decimal: bool = True
) -> list[tuple[str, str]]:
        ...
        if has_int:
            if allow_decimal and j < n and s[j] == "." and (j + 1 < n and s[j + 1].isdigit()):
                ...
                while j2 < n and s[j2].isdigit():
                    j2 += 1
                out.append((s[i:j2], "number"))
                i = j2
                continue
            out.append((s[i:j], "number"))
            i = j
            continue
...
# ---- skill entrypoint ----
def word_count(text: str | None = None, **kw: Any) -> dict[str, Any]:
    ...
    for tok, kind in toks:
        if kind == "word":
            if not _script_accepts(tok, script):
                continue
